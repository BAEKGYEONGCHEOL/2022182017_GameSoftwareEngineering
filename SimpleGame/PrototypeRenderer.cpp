...
PrototypeRenderer::PrototypeRenderer(int width, int height)
	: m_Initialized(false), m_Width(width), m_Height(height), m_Program(0), m_PostProgram(0), m_BloomProgram(0),
	  m_VertexBuffer(0), m_SceneFramebuffer(0), m_SceneTexture(0), m_PositionAttribute(-1),
	  m_ScreenSizeUniform(-1), m_ColorUniform(-1), m_FontHandle(NULL)
{
	...
	if (m_Program == 0 || m_PostProgram == 0 || m_BloomProgram == 0) return;
	...
	if (m_VertexBuffer == 0 || m_PositionAttribute < 0 || m_ScreenSizeUniform < 0 || m_ColorUniform < 0 || !CreateSceneTarget())
	{
...
{
	for (std::unordered_map<wchar_t, GLuint>::const_iterator glyph = m_Glyphs.begin(); glyph != m_Glyphs.end(); ++glyph)
		glDeleteLists(glyph->second, 1);
	if (m_FontHandle != NULL) DeleteObject(static_cast<HFONT>(m_FontHandle));
	DestroySceneTarget();
	if (m_VertexBuffer != 0) glDeleteBuffers(1, &m_VertexBuffer);
	if (m_Program != 0) glDeleteProgram(m_Program);
	if (m_PostProgram != 0) glDeleteProgram(m_PostProgram);
	if (m_BloomProgram != 0) glDeleteProgram(m_BloomProgram);
}

bool PrototypeRenderer::IsInitialized() const { return m_Initialized; }
int PrototypeRenderer::Width() const { return m_Width; }
int PrototypeRenderer::Height() const { return m_Height; }
...
void PrototypeRenderer::Present(float timeSeconds)
{
	...
	glUniform2f(glGetUniformLocation(m_BloomProgram, "u_Resolution"), static_cast<float>(m_Width), static_cast<float>(m_Height));
...
void PrototypeRenderer::DrawQuad(const ScreenPoint& a, const ScreenPoint& b, const ScreenPoint& c,
	const ScreenPoint& d, float r, float g, float bColor, float alpha)
{
	const float vertices[] = { a.x, a.y, b.x, b.y, c.x, c.y, a.x, a.y, c.x, c.y, d.x, d.y };
	DrawVertices(vertices, 6, r, g, bColor, alpha);
}

void PrototypeRenderer::DrawRect(float x, float y, float width, float height, float r, float g, float b, float a)
{
...
void PrototypeRenderer::DrawDiamond(float x, float y, float width, float height, float r, float g, float b, float a)
{
...
void PrototypeRenderer::DrawString(float x, float y, const std::string& text, float r, float g, float b, float a)
{
	std::wstring wideText(text.begin(), text.end());
	DrawString(x, y, wideText, r, g, b, a);
}

void PrototypeRenderer::DrawString(float x, float y, const std::wstring& text, float r, float g, float b, float a)
{
	if (m_FontHandle == NULL) return;
	glUseProgram(0);
	...
	glColor4f(r, g, b, a);
	glRasterPos2f(x, y);
	for (std::wstring::const_iterator character = text.begin(); character != text.end(); ++character)
	{
		GLuint glyph = GetGlyph(*character);
		if (glyph != 0) glCallList(glyph);
	}
	glPopMatrix();
	glMatrixMode(GL_PROJECTION);
	glPopMatrix();
...
{
	HFONT font = CreateFontW(-18, 0, 0, 0, FW_MEDIUM, FALSE, FALSE, FALSE, HANGUL_CHARSET,
		OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, FIXED_PITCH | FF_MODERN, L"Malgun Gothic");
	if (font == NULL)
		font = CreateFontW(-18, 0, 0, 0, FW_MEDIUM, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
			OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH, L"Arial");
	m_FontHandle = font;
	return font != NULL;
}
...
GLuint PrototypeRenderer::GetGlyph(wchar_t character)
{
	...
	if (existing != m_Glyphs.end()) return existing->second;
	if (m_FontHandle == NULL) return 0;
...
bool PrototypeRenderer::ReadFile(const char* filename, std::string& result) const
{
	...
	std::string line;
	while (std::getline(file, line)) { result += line; result += "\n"; }
	return true;
}
...
{
	const char* vertexPaths[] = { "./Shaders/Prototype.vs", "./SimpleGame/Shaders/Prototype.vs", "../../SimpleGame/Shaders/Prototype.vs" };
	const char* fragmentPaths[] = { "./Shaders/Prototype.fs", "./SimpleGame/Shaders/Prototype.fs", "../../SimpleGame/Shaders/Prototype.fs" };
	std::string vertexSource, fragmentSource;
	for (int i = 0; i < 3 && vertexSource.empty(); ++i)
	{
		std::string candidateVertex, candidateFragment;
		if (ReadFile(vertexPaths[i], candidateVertex) && ReadFile(fragmentPaths[i], candidateFragment))
		{
...
{
	const char* vertexPaths[] = { "./Shaders/PrototypePost.vs", "./SimpleGame/Shaders/PrototypePost.vs", "../../SimpleGame/Shaders/PrototypePost.vs" };
	const char* fragmentPaths[] = { "./Shaders/PrototypePost.fs", "./SimpleGame/Shaders/PrototypePost.fs", "../../SimpleGame/Shaders/PrototypePost.fs" };
	std::string vertexSource, fragmentSource;
	for (int i = 0; i < 3 && vertexSource.empty(); ++i)
	{
		std::string candidateVertex, candidateFragment;
...
{
	const char* vertexPaths[] = { "./Shaders/PrototypePost.vs", "./SimpleGame/Shaders/PrototypePost.vs", "../../SimpleGame/Shaders/PrototypePost.vs" };
	const char* fragmentPaths[] = { "./Shaders/PrototypeBloom.fs", "./SimpleGame/Shaders/PrototypeBloom.fs", "../../SimpleGame/Shaders/PrototypeBloom.fs" };
	std::string vertexSource, fragmentSource;
	for (int i = 0; i < 3 && vertexSource.empty(); ++i)
	{
		std::string candidateVertex, candidateFragment;
	...
	GLint success = GL_FALSE;
	glGetProgramiv(program, GL_LINK_STATUS, &success);
	if (success == GL_FALSE) { glDeleteProgram(program); return 0; }
	return program;
}
...
void PrototypeRenderer::DestroySceneTarget()
{
	for (int i = 0; i < 2; ++i)
	{
		if (m_BloomTextures[i] != 0) glDeleteTextures(1, &m_BloomTextures[i]);
		if (m_BloomFramebuffers[i] != 0) glDeleteFramebuffers(1, &m_BloomFramebuffers[i]);
		m_BloomTextures[i] = 0;
